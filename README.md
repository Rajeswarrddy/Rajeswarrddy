Hi, I'm Rajeswar Reddy, a Clooud Engineer with 3+ years of experience in AWS, Java, Spring Boot, DevOps, and cloud-native applications.

☁️ Experienced with AWS , Azure, Docker, Kubernetes, and Jenkins.
💻 Build Java/Spring Boot microservices and REST APIs.
🚀 Interested in Cloud, DevOps, and scalable applications.
🔧 Enjoy solving technical problems and improving application performance.
🎓 Master's in Information Systems and Technology.

I'm always learning and exploring new technologies to build better and more reliable cloud applications.

## 🛠️ Technologies & Skills

### ☁️ Cloud

`AWS` `Azure` `EC2` `S3` `IAM` `Lambda` `RDS` `AKS` `ACR`

### 💻 Backend

`Java` `Spring Boot` `Microservices` `REST APIs` `Hibernate` `JPA`

### ⚙️ DevOps & CI/CD

`Jenkins` `Docker` `Maven` `Ansible` `CI/CD`

### ☸️ Containers & Orchestration

`Kubernetes` `AKS` `ACR`

### 🏗️ Infrastructure as Code

`Terraform`

### 📊 Monitoring & Observability

`Prometheus` `Grafana`

### 🎨 Frontend

`JavaScript` `React.js` `Angular` `HTML5` `CSS3` `jQuery`

### 🗄️ Databases

`PostgreSQL` `MySQL` `Oracle`

### 🔧 Tools & Version Control

`Git` `Bitbucket` `Postman` `IntelliJ IDEA` `Eclipse` `Confluence`

### 🐍 Programming

`Python`

## 🚀 Currently Interested In

* ☁️ Cloud & Cloud-Native Technologies
* ⚙️ DevOps & CI/CD Automation
* ☸️ Kubernetes & Containerization
* 🏗️ Infrastructure as Code with Terraform
* 📊 Monitoring & Observability
* 🔐 Scalable and Reliable Applications

---

⭐ Feel free to explore my repositories and connect with me!

